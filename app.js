// File: app.js
// VERSI DENGAN ALERT SUKSES YANG LEBIH BAIK
...
    const validationOkButton = document.getElementById('validation-ok-button');
    ...
    // BARU: Ambil elemen judul modal validasi
...
                renderCart();
                // BARU: Panggil modal sukses
                showAppModal("Produk ditambahkan ke keranjang!", 'success');
                return; 
            }
        }
...
            itemRow.innerHTML = `<img src="${item.image_url}" alt="${product.name}"><div class="cart-item-info"><b>${item.name}</b><span>${priceDisplay}</span>${detailsHtml}</div><div class="quantity-controls"><button class="quantity-down" data-id="${item.uniqueCartId}">-</button><input type="number" value="${item.quantity}" min="1" data-id="${item.uniqueCartId}"><button class="quantity-up" data-id="${item.uniqueCartId}">+</button></div><span class="item-subtotal"><b>${formatRupiah(item.price * item.quantity)}</b></span><button class="remove-item-button" data-id="${item.uniqueCartId}">&times;</button>`;
...
            
            if (errorMessage) { 
                // BARU: Panggil modal error
                showAppModal(errorMessage, 'error'); 
          _       return; 
            }
...
  m   } catch (err) {
            // BARU: Panggil modal error
            showAppModal('Terjadi kesalahan: ' + err.message, 'error');
            checkoutButton.disabled = false;
            checkoutButton.textContent = 'Proses Pesanan';
        }
    });
...
    // --- 6. Fungsi Modal Validasi/Sukses ---
    
    // BARU: Fungsi ini menggantikan showValidationError
    function showAppModal(message, type = 'error') {
        validationMessageEl.textContent = message;

        if (type === 'success') {
            validationTitleEl.textContent = 'Sukses!'; // Judul baru
            validationTitleEl.style.color = '#28a745'; // Warna hijau
        } else {
            validationTitleEl.textContent = 'Mohon Lengkapi Data'; // Judul error
            validationTitleEl.style.color = '#dc3545'; // Warna merah
        }
